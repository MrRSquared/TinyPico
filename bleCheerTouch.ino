 ...
 *  Touch code was modified from Rui Santos' tutorial https://randomnerdtutorials.com/esp32-touch-pins-arduino-ide/
 *  */
...
void loop() {
   ...
   touchValueUp = touchRead(touchPin1);
  touchValueDown = touchRead(touchPin2);
  //Serial.print(touchValue);
  // check if the touchValue is below the threshold
  // if it is, set ledPin to HIGH
  if(touchValueUp < threshold){
    // turn LED on
    brightness += .1;
    
    Serial.println(touchValueUp);
  }
  if(touchValueDown < threshold){
    // turn LED off
    brightness -= .1;
    
    Serial.println(touchValueDown);
  }
  if (brightness < 0){
    brightness = 0;
  }
